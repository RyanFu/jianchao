...
public class ModifyBankCardInfoAcitivity extends BaseActivity implements
		OnClickListener {
...
//	private String strBranch;// 开户行
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.peopleinfo_update_bankcard);
...
//			}
	...
	public void dohttpUpdatebank() {

		JsonBuilder jsonBuilder = new JsonBuilder();
		jsonBuilder.put("id", bank_id);

		BaseHttpClient.post(getBaseContext(), Default.edit,
							...
							if (statusCode == 200) {
								if (json.getInt("status") == 1) {
									...
									if (json.has("mobile")) {
										...
										mUser_phone.setText(json.optString("mobile", "0") );
									}
									if (json.has("bank_name")) {

										bank_name.setText(json.optString("bank_name", "0") );
//                                        strBank = bank_name.getText().toString();
									}
									if (json.has("bank_num")) {

										mCardNumber.setText(json.optString("bank_num", "0") );
									}
									if (json.has("province")) {

										province_name.setText(json.optString("province", "0") );
//                                        strProvince = province_name.getText().toString();
									}
									if (json.has("city")) {

										city_name.setText(json.optString("city", "0") );
//                                        strCity = city_name.getText().toString();
									}
									if (json.has("bank_address")) {

										mBankName.setText(json.optString("bank_address", "0") );
									}
									...
									SystenmApi.showCommonErrorDialog(ModifyBankCardInfoAcitivity.this, json.getInt("status"),message);
								}
							} else {
...
//											mhandle.postDelayed(runnbale, 1000);
											...
											SystenmApi.showCommonErrorDialog(ModifyBankCardInfoAcitivity.this, response.getInt("status"),message);
										}
									}
								} else {
...
//            bank_select.setTitle("选择银行");
        ...
        for(int i=0;i<mProvinceList.size();i++){
            ...
            if(mProvinceList.get(i).name.equals(strProvince)){


                dohttpCity(mProvinceList.get(i).id);
            }
	...
	public void doHttp() {
		...
		BaseHttpClient.post(getBaseContext(), Default.doedit,
				jsonBuilder, new JsonHttpResponseHandler() {
