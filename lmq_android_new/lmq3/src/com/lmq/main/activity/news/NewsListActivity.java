	...
	protected void initView() {
		...
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				Intent intent = new Intent(NewsListActivity.this,LMQWebViewActivity.class);
						...
						try {
							...
							if (statusCode == 200) {
								if (response.getInt("status") == 1) {
									...
									SystenmApi.showCommonErrorDialog(NewsListActivity.this, response.getInt("status"),message);
								}
							} else {
	...
	class newsAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(null == convertView){
				...
				LayoutInflater mInflater = LayoutInflater.from(NewsListActivity.this);
						...
						try {
							...
							if (statusCode == 200) {
								if (json.getInt("status") == 1) {
									...
									SystenmApi.showCommonErrorDialog(NewsListActivity.this, json.getInt("status"),message);
								}
							} else {
								showCustomToast(R.string.toast1);
							}
