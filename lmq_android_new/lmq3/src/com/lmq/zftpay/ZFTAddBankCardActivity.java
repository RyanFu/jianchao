...
public class ZFTAddBankCardActivity extends BaseActivity implements
		OnClickListener {
	
	private LayoutInflater mInflater;
//	private Spinner mBank, mProvince, mCity;
	private EditText  mCardNumber, mBankName;
...
//											SystenmApi.showCommonErrorDialog(ZFTAddBankCardActivity.this, response.getInt("status"),message);
//										}
//									}
//								} else {
//
					...
					public void onSuccess(int statusCode, Header[] headers,
						...
						try {
							if (statusCode == 200) {
								...
								SystenmApi.showCommonErrorDialog(ZFTAddBankCardActivity.this, json.getInt("status"),message);
							}
						} catch (Exception e) {
							e.printStackTrace();
						}
	...
	public void getCityInfo(JSONObject json) {
		try {
			...
			for (int i = 0; i < citys.length(); i++) {
				...
				mCityList.add(item);
                city_select_list.add(item.value);
			}
...
//		.setText(mobile);
		...
		if(json.has("real_name")){
			tv_real_name.setText(json.optString("real_name"));
		}
		if(json.has("mobile")){
			mUser_phone.setText(json.optString("mobile"));
		}
		...
		try {
			JSONArray banks = json.getJSONArray("bank_list");
			...
			for (int i = 0; i < banks.length(); i++) {
				...
				BankItem item = new BankItem(temp);
				mBankList.add(item);
                bank_select_list.add(item.value);
...
//            bank_select.setTitle("选择银行");
	...
	public void doHttp() {
		...
		jsonBuilder.put("userName", tv_real_name.getText().toString());
		jsonBuilder.put("bank_name", strBank);// 银行卡
		jsonBuilder.put("bank_num", mCardNumber.getText().toString());// 卡号
		jsonBuilder.put("bank_province", province_name.getText().toString());// 开户省  strProvince
		jsonBuilder.put("bank_city", city_name.getText().toString());// 城市  strCity
		jsonBuilder.put("card_type", 0);
		jsonBuilder.put("bank_address", mBankName.getText().toString());// 开户行
        jsonBuilder.put("bank_phone",mUser_phone.getText().toString());
