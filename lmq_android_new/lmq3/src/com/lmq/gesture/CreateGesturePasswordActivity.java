...
import com.lmq.main.util.LockPatternView.DisplayMode;
...
public class CreateGesturePasswordActivity extends Activity implements
	...
	private boolean modify_pass_flg = false;
	/**
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		text.setText(R.string.screen_lock_creat);

		modify_pass_flg = getIntent().getBooleanExtra("modify_pass", false);
	...
	public void finish() {
		...
		if (modify_pass_flg) {
			...
			startActivity(new Intent(CreateGesturePasswordActivity.this,
	...
	private void updatePreviewViews() {
		if (mChosenPattern == null)
			return;
		MyLog.i("way", "result = " + mChosenPattern.toString());
		for (LockPatternView.Cell cell : mChosenPattern) {
			MyLog.i("way", "cell.getRow() = " + cell.getRow()
	...
	protected LockPatternView.OnPatternListener mChooseNewLockPatternListener = new LockPatternView.OnPatternListener() {
		...
		public void onPatternDetected(List<LockPatternView.Cell> pattern) {
			if (pattern == null)
				return;
			// MyLog.i("way", "result = " + pattern.toString());
	...
	private void saveChosenPatternAndFinish() {
		...
		editor.putBoolean("sl", true);
		editor.commit();
		LocationApplication.getInstance().getLockPatternUtils()
				.saveLockPattern(mChosenPattern);
		showToast("密码设置成功");
		Default.passwordSwitchChanged = true;
		// MainMenuActivity.settingFragment.setpwdSwitch();
		// setResult(1212);
